    ...
    public:
        ...
        DA2Network(const char *network_path) {
            ...
            // Enable CUDA (GPU) if available
                ...
                printf("Using GPU for Depth Analysis\n");
            #endif

            // Convert char* to wchar_t* for Windows
            wchar_t wnetwork_path[256];
            mbstowcs(wnetwork_path, network_path, 256);
            
            // Set up the Ort session
            this->session_ = new Ort::Session(env, wnetwork_path, session_options);
        }
